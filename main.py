...
def main(opts):
    ...
    feature, img_spec = load_features(opts.img_feat_dir)#img_spec = w, h, vfov etc..
    ...
    if opts.test_submission:
        ...
        agent_kwargs = {
            ...
            'env': test_env[1][0],
            'results_path': "",
            'encoder': encoder,#EncoderRNN
            'model': model,#SelfMonitoring
    ...
    #best_success_rate_seen = best_success_rate_seen if opts.resume else 0.0
    ...
    for epoch in range(opts.start_epoch, opts.max_num_epochs + 1):
        ...
        if epoch % opts.eval_every_epochs == 0:
            ...
            success_rate_compare = success_rate[0]#Changed to 0 (val_seen Env)
            #success_rate_seen_compare = success_rate[0]

            if is_experiment():
                # remember best val_seen success rate and save checkpoint???? THIS IS UNSEEN
                is_best = success_rate_compare >= best_success_rate
                best_success_rate = max(success_rate_compare, best_success_rate)
                print("--> Highest val_seen success rate: {}".format(best_success_rate))#unseen
                ...
                is_best_seen = success_rate_seen_compare >= best_success_rate_seen
                best_success_rate_seen = max(success_rate_seen_compare, best_success_rate_seen)
                print('--> Highest val_seen success rate: {}'.format(best_success_rate_seen))

                # save the model if it is the best so far
                save_checkpoint_val_seen({
                    'opts': opts,
                    'epoch': epoch + 1,
                    'state_dict': model.state_dict(),
                    'encoder_state_dict': encoder.state_dict(),
                    'best_success_rate_seen': best_success_rate_seen,
                    'optimizer': optimizer.state_dict(),
                    'max_episode_len': opts.max_episode_len,
                }, is_best_seen, checkpoint_dir=opts.checkpoint_dir, name=opts.exp_name)
                '''

...
from json_read import json_read_main
from utils import load_datasets, load_nav_graphs, print_progress, is_experiment

csv.field_size_limit(sys.maxsize)

#Make the Image feature from the tsv file(pre-extracted image feature file) --Jisu
def load_features(feature_store):
    def _make_id(scanId, viewpointId):
        return scanId + '_' + viewpointId

    # if the tsv file for image features is provided ==> Read the File! --Jisu
    if feature_store:
...
class PanoEnvBatch():
    ...
    # This is used in R2RPanoBatch (self.env.getState, self.env.newEpisodes .... etc) --Jisu

    def __init__(self, features, img_spec, batch_size=64):
        self.features = features #for image feature --Jisu
        self.image_w, self.image_h, self.vfov = img_spec #These are in tsv file --Jisu
    ...
    def newEpisodes(self, scanIds, viewpointIds, headings):
        ...
        # This make environment(action?) pairs (scanid,viewpointid,heading)

    def getStates(self):
        """ Get list of states augmented with precomputed image features. rgb field will be empty. """
        feature_states = []
        #sim : getState == location, scanId, heading, elevation, step, navigableLocation --Jisu -> src/lib/MatterSim.cpp
        ...
        # features --> dictionary shape : ScanId_Viewpoint_id : [feature] --Jisu
...
class R2RPanoBatch():
    ...
    def _get_obs(self):
        ...
        for i, (feature, state) in enumerate(self.env.getStates()):
            ...
            (object_t, place_t) = json_read_main(state.scanId, state.location.viewpointId)
    ...
    def teleport_beam(self, batch_idx, scanIds, viewpointIds, headings):
        ...
        (object, place) = json_read_main(state.scanId, state.location.viewpointId)

...
def save_checkpoint_val_seen(state, is_best, checkpoint_dir='checkpoints/val_seen/', name='checkpoint'):
    os.makedirs(checkpoint_dir, exist_ok=True)
    filename = checkpoint_dir + name + '.pth.tar'
    torch.save(state, filename)
    if is_best:
...
def resume_training(opts, model, encoder, optimizer):
    if opts.resume == 'latest':
        ...
        file_extention = '_model_best.pth.tar'#file_extention = '_model_best.pth.tar'
    else:
